// Package blake2b256 implements the unkeyed BLAKE2b-256 digest used by Sui
// address derivation.
package blake2b256
...
// Sum256 returns the unkeyed BLAKE2b digest of data with a 256-bit output.
...
func compress(h *[8]uint64, block *[blockSize]byte, counter uint64, final bool) {
	var m [16]uint64
	for i := range m {
	...
	v := [16]uint64{
		...
		iv[0], iv[1], iv[2], iv[3], iv[4] ^ counter, iv[5], iv[6], iv[7],
	}
	if final {
		v[14] = ^v[14]
	}
