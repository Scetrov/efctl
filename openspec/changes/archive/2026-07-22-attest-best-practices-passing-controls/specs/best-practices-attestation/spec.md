...
The repository SHALL document that `.bestpractices.json` supplies proposed answers only and that an authorized project editor must review and save them for bestpractices.dev project 13754. The README SHALL display the service's live project badge rather than a hard-coded passing image or statement.

#### Scenario: Implementation is merged
- **WHEN** the implementation PR reaches the default branch
- **THEN** the documented process directs an authorized editor to review, save, and verify the proposed answers on bestpractices.dev
